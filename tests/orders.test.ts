...
import { issueTestDecision } from "../src/automation/rules.js";
...
describe("OrderService local mock flow", () => {
  it("submits an approved decision only through mock providers and records all order fields", () => {
    ...
    expect(result).toMatchObject({ productIdentifier: "mock-milk-1", productName: "Milk (1 L)", quantity: 2, unitPricePaise: 6800, totalPaise: 13600, decision: true, reason: DecisionReason.APPROVED, status: OrderStatus.SUBMITTED });
  ...
  it("prevents duplicate order creation after a mock submission", () => {
    const setup = orderService();
    expect(setup.service.process(approvedDecision()).status).toBe(OrderStatus.SUBMITTED);
